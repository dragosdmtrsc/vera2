...
action handle_2b() {
    ...
    shift_left(paxos.tmp, 1, paxos.acptid);
    modify_field(local_metadata.tmp_8, paxos.tmp);
    bit_or(local_metadata.acceptors, local_metadata.tmp_8, local_metadata.acceptors);
...
action handle_new_value() {
    ...
    register_write(values_register, paxos.inst, paxos.paxosval);
    shift_left(paxos.tmp, 1, paxos.acptid);
    register_write(history2B, paxos.inst, paxos.tmp);
}
table learner_tbl {
