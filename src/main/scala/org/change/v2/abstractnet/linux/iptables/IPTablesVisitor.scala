...
class RuleVisitor extends BaseVisitor {
  private var instructions : List[Instruction] = List[Instruction]()
  def getMatches = instructions
  
  private var instruction : Instruction = NoOp
  def getTarget = instruction
  override def visit(target : IPTablesRule) {
    val targetMatcher = new TargetVisitor
    target.getIPTablesTarget().accept(targetMatcher)
    this.instruction = targetMatcher.getInstruction
  }
  ...
  override def visit(option : FragmentOption) {
    ...
    List(Constrain(Tag("IPFragmentBit"), if (option.getNeg) :~:(constraint) else constraint))
  }
...
class TargetVisitor extends BaseVisitor {
  ...
  override def visit(target : SNATTarget) {
    this.instruction = InstructionBlock(Assign("SrcIp", :@(Tag("IPSrc"))),
        Assign("SrcPort", :@(Tag("L4Port"))),
