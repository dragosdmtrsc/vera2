...
import org.change.v2.analysis.processingmodels.{LocationId, _}
...
case class MapState(location: LocationId,
  ...
  def step(l: LocationId): MapState = if (topology.contains(l)) new MapState(l, topology, links, state, portHistory)
    ...
    Policy.verbose_print("Next hop:" + links(l), OverallMode); new MapState(links(l), topology, links, state, portHistory)
  }
