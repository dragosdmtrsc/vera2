...
object Policy_v2 {
  ...
  def boolean_check(f: Formula, s: State, check: (Formula, State) => Formula): Formula = {
    ...
    if (f.status != Pending) {
      verbose_print("already verified"); return f
  ...
  def make(f: Formula, s: Status): Formula = {
    f.status = s; f
  }
  ...
  def check(o: Formula, p: Instruction, s: State): Formula = {
    ...
    (o, p) match {
      ...
      case (_, Fork(pr :: rest)) => verbose_print("Checking branch:\n" + show(pr));
        (o, check(o, pr, s).status) match { //execute the first branch of the Fork
  ...
  // "top-level" verification procedure for SEFL code
