...
object Policy {
  ...
  def boolean_check(f: Formula, s: PolicyState, check: (Formula, PolicyState) => Formula): Formula = {
    ...
    if (f.status != Pending) {
      verbose_print("already verified", MCMode); return f
    } //this formula was already verified
  ...
  def make(f: Formula, s: Status, st: PolicyState): Formula = {
    f.status = s; f.state = st; f
  }

  def clone(f: Formula, st: PolicyState): Formula = {
    f.state = st; f
  }

  def clear(f: Formula): Formula = {
    f.clear_valuation(); f
  }
  ...
  */
    ...
    (s, f, p) match {
      ...
      => check(f,
        ...
        , s)
      case (_, _, InstructionBlock(If(test, thenWhat, els) :: rest)) => check(f, InstructionBlock(transform(s, If(test, thenWhat, els)) :: rest), s)
