...
object P4PrettyPrinter {
  implicit def toDotable(controlFlowInterpreter: ControlFlowInterpreter) = ToDot(controlFlowInterpreter.switchInstance.getName,
    controlFlowInterpreter.instructions(),
    controlFlowInterpreter.links())
  implicit def toVisable(controlFlowInterpreter: ControlFlowInterpreter) = ToVis(controlFlowInterpreter.instructions(),
    controlFlowInterpreter.links())
