...
class P4GrammarListener extends P4GrammarBaseListener {
  ...
  override def exitArray_instance(ctx: P4GrammarParser.Array_instanceContext): Unit = {
    ...
    val headerType = ctx.header_type_name().getText
    ctx.arrInstance = new ArrayInstance(headerType, instanceName, index.intValue())
  ...
  override def exitField_list_calculation_declaration(ctx: Field_list_calculation_declarationContext): Unit = {
    switch.declareCalculation(
      new Calculation(ctx.field_list_calculation_name().getText), false
    )
  }
  ...
  override def exitCounter_declaration(ctx: Counter_declarationContext): Unit = {
    ...
    ).setCounter(true).setName(ctx.counter_name().getText)
  ...
  override def exitExtract_or_set_statement(ctx: P4GrammarParser.Extract_or_set_statementContext): Unit = {
    ctx.functionStatement = Option(ctx.extract_statement()).map(_.extractStatement).orElse(
        Some(SetFunction(ctx.set_statement().field_ref().getText,
          ctx.set_statement().exp().getText))
      ).get
  ...
  override def exitAction_function_declaration(ctx: Action_function_declarationContext): Unit = {
    val complexAction = new P4ComplexAction(ctx.action_header().action_name().getText)
    if (ctx.action_header() != null && ctx.action_header().param_list() != null &&
  ...
  override def exitAction_statement(ctx: Action_statementContext): Unit = {
    ctx.actionCall = new P4ActionCall(new P4Action(P4ActionType.UNKNOWN,
      ctx.action_name().getText))
    for (a <- ctx.arg())
  ...
  override def exitArg(ctx: ArgContext): Unit = {
    if (ctx.field_ref() != null) {
      ...
      ctx.expr = new LiteralExpr(ctx.field_value().fieldValue,
  ...
  override def enterP4_program(ctx : P4_programContext) : Unit = {
    ...
    val metadataInstancej = new model.HeaderInstance("standard_metadata_t",
      "standard_metadata").setMetadata(true)
    switch.declareHeader(hOffs.foldLeft(new Header().setName("standard_metadata_t").setLength(512))((acc, x) => {
      acc.addField(new Field().setLength(x._2._2).setName(x._2._1))
    }), false).declareHeaderInstance(metadataInstancej, false)
  }
  ...
  override def exitMeter_declaration(ctx: Meter_declarationContext): Unit = {
    ctx.spec = new RegisterSpecification().setName(ctx.meter_name().getText).setMeter(true)
    switch.declareRegister(ctx.spec, false)
  }
  ...
  override def exitField_list_entry(ctx: Field_list_entryContext): Unit = {
    if (ctx.field_ref() != null)
      ctx.entry = new fieldlists.FieldList.FieldRefEntry(ctx.field_ref().expression)
    else if (ctx.header_ref() != null) {
      //need to disambiguate on reference solving
      ctx.entry = new fieldlists.FieldList.StringRefEntry(ctx.header_ref().expression.getPath)
    } else if (ctx.field_list_name() != null) {
      //need to disambiguate on reference solving
      ctx.entry = new fieldlists.FieldList.StringRefEntry(ctx.field_list_name().getText)
    } else if (ctx.field_value() != null) {
  ...
  override def exitTable_declaration(ctx: Table_declarationContext): Unit = {
    ...
    if (ctx.table_actions().action_specification() != null &&
      ...
      ctx.tableDeclaration.setProfile(ctx.table_actions()
  ...
  override def exitField_match(ctx : Field_matchContext) : Unit = {
    ctx.tableMatch = new TableMatch(ctx.tableName,
      ...
      ctx.field_or_masked_ref().mask).setReferenceKey(ctx.field_or_masked_ref().expression)
  }
  ...
  override def exitApply_table_call(ctx:Apply_table_callContext) {
    ...
    this.tableSelectors((ctx.table_name().getText, execId.toString)) = false
  }
  ...
  override def exitApply_and_select_block(ctx:Apply_and_select_blockContext){
    ...
    this.tableSelectors((ctx.table_name().getText, execId.toString)) = true
    val defaultEntry = ctx.case_list().instructions.collectFirst {
      case v@If(ConstrainNamedSymbol(what, _, _), b, _) if what == "default.Fired" => b
    }.getOrElse(Forward(s"${ctx.parent}.out"))
  ...
  override def exitHit_miss_case(ctx:Hit_miss_caseContext){
    ctx.caseEntry = TableCaseEntry.from(ctx.hit_or_miss().getText,
      ctx.control_block().blockStatement)
    ctx.instruction = If (Constrain(".Hit", :==:(ConstantValue(if (ctx.hit_or_miss().getText == "hit") 1 else 0))),
  ...
  override def exitAction_case(ctx:Action_caseContext){
    ctx.caseEntry = TableCaseEntry.from(ctx.action_or_default().getText,
      ctx.control_block().blockStatement)
  ...
  override def exitIf_else_statement(ctx:If_else_statementContext){
    ...
    if (ctx.else_block() != null) {
      ctx.ifelseStatement = new IfElseStatement(ctx.bool_expr().bexpr,
        ctx.control_block().blockStatement,
  ...
  override def exitCompound_exp(ctx:Compound_expContext): Unit ={
    ...
    ctx.bvexpr = BinExpr.from(ctx.bin_op().getText, ctx.exp(0).bvexpr, ctx.exp(1).bvexpr)
  }
  ...
  override def exitValue_exp(ctx:Value_expContext){
    ...
    ctx.bvexpr = new LiteralExpr(ctx.value().const_value().constValue,
  ...
  override def exitCompound_bool_expr(ctx:Compound_bool_exprContext) {
    ctx.bexpr = BinBExpr.from(ctx.bool_op().getText,
  ...
  override def exitRelop_bool_expr(ctx:Relop_bool_exprContext){
    ctx.bexpr = RelOp.from(ctx.rel_op().getText, ctx.exp(0).bvexpr, ctx.exp(1).bvexpr)
  }
  ...
  override def exitLatest_field_ref_expr(ctx: Latest_field_ref_exprContext): Unit = {
  ...
  override def exitReturn_statement(ctx: Return_statementContext): Unit = {
    if (ctx.return_value_type() != null)
      ...
      ctx.statement = ctx.case_entry().foldLeft(new ReturnSelectStatement())((acc, x) => {
        acc.add(ctx.select_exp().bvexpressions.foldLeft(x.caseEntry)((acc2, y) => {
