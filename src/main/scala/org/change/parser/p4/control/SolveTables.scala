...
import org.change.v2.p4.model.control.{ApplyAndSelectTableStatement, ApplyControlStatement, ApplyTableStatement}
...
class SolveTables(switch: Switch) extends ASTVisitor {
  override def postorder(applyControlStatement : ApplyControlStatement): Unit = {
    applyControlStatement.setBlock(switch.controlBlock(applyControlStatement.getBlock.getName))
  }
  ...
  def solve(expr : Expression) : Unit = expr match {
    case hr : HeaderRef => if (hr.getInstance() == null) {
      val inst = switch.getInstance(hr.getPath)
      if (inst == null)
        throw new NoSuchFieldError(s"can't resolve header ${hr.getPath}")
      hr.setInstance(inst)
    }
    case fr : FieldRef => solve(fr.getHeaderRef)
      val fld = fr.getHeaderRef.getInstance().getLayout.getField(fr.getField)
      if (fld == null) {
        throw new NoSuchFieldError(s"can't resolve field ${fr.getField} in header ${fr.getHeaderRef.getInstance()}")
      }
