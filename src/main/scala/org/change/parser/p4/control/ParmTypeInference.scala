...
import org.change.v2.p4.model.parser.{FieldRef, ParmRef}
...
class ParmTypeInference(switch: Switch) extends ASTVisitor {
  ...
  def handleTriple(actCall : P4ActionCall): Unit = {
    ...
    val kinds = actCall.params()
      .asScala.map(_.asInstanceOf[ParamExpression])
  ...
  override def postorder(actCall: P4ActionCall): Unit = {
    val params = actCall.params().asScala.map(_.asInstanceOf[ParamExpression]).collect {
      case pe : ParamExpression if pe.getExpression.isInstanceOf[ParmRef] => pe
      case pe : ParamExpression if pe.getExpression.isInstanceOf[LiteralExpr] => pe
    }
    ...
    actCall.getP4Action match {
      ...
      case ax : org.change.v2.p4.model.actions.primitives.Truncate =>
        ...
        typeSolver.equal(actCall.params().get(0).asInstanceOf[ParamExpression],
          BVType(16))
      case ax : org.change.v2.p4.model.actions.P4ComplexAction =>
        assert(all.size == ax.getParameterList.size())
        all.zip(ax.getParameterList.asScala).foreach(p => {
    ...
    typeSolver.solve {
      ...
      x._1 match {
        ...
        case pe: ParamExpression
          ...
          if (x._2.nonEmpty) {
            x._2.get match {
              case BVType(w) => pe.getExpression.asInstanceOf[LiteralExpr].setWidth(w)
              case _ => ;
            }
          } else {
