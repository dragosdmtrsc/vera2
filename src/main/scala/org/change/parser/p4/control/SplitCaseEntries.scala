...
import org.change.v2.p4.model.control.exp.{FieldRefExpr, LiteralExpr, P4Expr}
...
class SplitCaseEntries(switch : Switch) extends ASTVisitor {
  ...
  def splitANumber(caseEntry : CaseEntry, v : BigInt) : List[LiteralExpr] = {
    ...
    caseEntry.getBVExpressions.asScala.reverse.foreach {
      ...
      case x : P4Expr => throw new IllegalArgumentException(s"can't handle anything but field ref exprs, got $x")
    }
    if (currentNumber != 0 && currentNumber != -1) {
      throw new AssertionError(s"unconsumed number $v -> only got to $currentNumber")
    }
    listOfNumbers
  }

  override def postorder(caseEntry: CaseEntry): Unit = {
    if (!caseEntry.isDefault && caseEntry.getBVExpressions.size() != caseEntry.getBvValues.size()) {
      if (caseEntry.getBvValues.size() != 1)
        throw new AssertionError("values size must be 1, got " + caseEntry.getBvValues)
