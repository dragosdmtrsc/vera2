...
import org.change.v2.p4.model.parser.{FieldRef, MaskedFieldRef}
...
class TablesToLogics(switch: Switch,
  ...
  override def preorder(tableDeclaration: TableDeclaration): Boolean = {
    val flowStruct = FlowStruct(context, tableDeclaration, switch)
    tabToLogic.put(tableDeclaration, flowStruct)
    false
  }
}

object TablesToLogics {
  def apply(switch : Switch,
            context: Z3Context): mutable.Map[TableDeclaration, FlowStruct] = {
    val mmap = mutable.Map.empty[TableDeclaration, FlowStruct]
    Traverse(new TablesToLogics(switch, context, mmap))(switch)
    mmap
  }
  def apply(switch : Switch,
            tableDeclaration: TableDeclaration,
            context: Z3Context): FlowStruct = {
    val mmap = mutable.Map.empty[TableDeclaration, FlowStruct]
    Traverse(new TablesToLogics(switch, context, mmap))(tableDeclaration)
    mmap(tableDeclaration)
  }
}
