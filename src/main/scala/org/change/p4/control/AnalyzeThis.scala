...
class AnalyzeThis(switch: Switch) {
  ...
  private val outstanding =
    ListBuffer.empty[(ControlStatement, Option[P4BExpr])]
  ...
  def lookAt(statement: ControlStatement): Unit = statement match {
    ...
    case ifElseStatement: IfElseStatement =>
      ...
      outstanding.clear()
      outstanding.append(
        (ifElseStatement, Some(new NegBExpr(ifElseStatement.getCondition)))
      )
    ...
    case applyControlStatement: ApplyControlStatement =>
      ...
      lookAt(
        switch.controlBlock(applyControlStatement.getBlock.getName).getStatement
      )
  }
  ...
  def build(
    ...
    if (ctrl != null) {
      ...
      if (controlName == "parser") {
        ...
        val eds = parserHelper.unrolledCFG
          .scc()
          .flatMap(r => {
            r.collect {
          ...
          .foldLeft(
            (
              Option.empty[ControlStatement],
              Map.empty[ControlStatement, List[ControlStatement]]
            )
          )((acc, x) => {
