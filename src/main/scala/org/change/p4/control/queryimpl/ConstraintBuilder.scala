...
object ConstraintBuilder {
  ...
  def mkInstance(context: Z3Context,
    ...
    var asts: ListBuffer[Z3AST] = ListBuffer(
      context.mkTrue(),
    ...
    actionSpec match {
      case Action(x) =>
        ...
        flowStruct.matchKinds.foreach(s => {
          asts.append(
            if (s.isBoolSort) context.mkFalse()
  ...
  def apply(switch: Switch,
            context: Z3Context,
            instance: Instance): Iterable[Z3AST] = {
    ...
    val cloneAxiom = if (clSpec != null) {
      ...
      val initialPredicate = context.mkEq(res, context.mkInt(511, res.getSort))
    ...
    val tableAxioms = switch
      ...
      .map(t => {
              ...
              f._1._1.getMatchKind match {
                ...
                case MatchKind.Lpm =>
                  ...
                  context.mkEq(
                    ...
                    context.mkBVAnd(
                      bound,
                      context.mkNumeral(msk.toString(), bound.getSort)
                    )
                  )
        ...
        val fullQuery = context.mkEq(flowStruct.query(bounds).z3AST, forallBody)
  ...
  def apply(switch: Switch,
            context: Z3Context,
            switchTarget: SwitchTarget): Iterable[Z3AST] = {
    val tm = TypeMapper()(context)
    Seq("constrain_ingress_port", "constrain_egress_port").map(f => {
      ...
      val body = context.mkOr(switchTarget.inputPorts.toList.map(p => {
        context.mkEq(const, tm.literal(funObj._1, p).z3AST)
      }): _*)
      context.mkForAllConst(
        0,
        Seq(),
        Seq(const),
