...
object MkQuery {
  def apply(ctx: P4Memory, expression: Expression): P4Query = expression match {
  ...
  def apply(context: P4Memory, expr: P4Expr): P4Query = {
    val where = mutable.Map.empty[Object, P4Query]
    Traverse(QueryMaker(context, where))(expr)
    where(expr)
  }
  def apply(context: P4Memory, expr: P4BExpr): P4Query = {
    val where = mutable.Map.empty[Object, P4Query]
    Traverse(QueryMaker(context, where))(expr)
    where(expr)
  }
  ...
  def indexOutOfBounds(context: P4Memory, expr: Expression): P4Query =
    expr match {
      case fr: FieldRef =>
