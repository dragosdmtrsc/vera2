...
import org.change.p4.model.control.exp.{DataRefExpr, FieldRefExpr}
    ...
    if (!caseEntry.isDefault) {
      if (caseEntry.getBVExpressions.size() != caseEntry.getBvValues.size()) {
        throw new AssertionError(
          caseEntry.getBVExpressions + " must be of equal size to " +
      ...
      caseEntry.getBVExpressions.asScala.zipWithIndex.foreach(idxd => {
        ...
        val flen: Long = e match {
          ...
          case _ =>
            throw new IllegalStateException(s"cannot have $e as a match")
        }
