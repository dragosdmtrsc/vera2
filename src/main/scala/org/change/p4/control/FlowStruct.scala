...
class FlowStruct(context: Z3Context, table: TableDeclaration, switch: Switch)
    extends P4Type {
  ...
  private val matchToSorts = table.getMatches.asScala
    .map(x => {
    ...
    .toList
  private val matchValueIndices =
    matchToSorts.foldLeft((0, Map.empty[TableMatch, (Int, Int)]))((acc, x) => {
      val (idx, crt) = acc
  ...
  val parmTypes: List[((String, String), Z3Sort)] = allowed
    .flatMap(act => {
      switch
        .action(act)
        .getParameterList
  ...
  private val parmsZipped: Map[(String, String), Int] =
    parmTypes.zipWithIndex.map(x => x._1._1 -> x._2).toMap
  ...
  private val structTypes =
    ...
    context.mkBoolSort() ::
      ...
      //action parameters
      parmTypes.map(_._2)
  val flowSort: (Z3Sort, Z3FuncDecl, Seq[Z3FuncDecl]) =
    context.mkTupleSort(s"FlowDef_${table.getName}", structTypes: _*)
  //query function
  val queryFun: Z3FuncDecl =
    context.mkFuncDecl(s"query_${table.getName}", queryParms, flowSort._1)
  ...
  def from(ast: Z3AST): FlowInstance = {
    if (ast.getSort != flowSort._1)
      throw new IllegalArgumentException(
        s"expecting ${flowSort._1}, got ${ast.getSort}"
      )
    FlowInstance(ast)
  }
...
object FlowStruct {
  private val flowStructLedger: mutable.Map[(Z3Context, String), FlowStruct] =
    mutable.Map.empty

  def apply(context: Z3Context,
            table: TableDeclaration,
            switch: Switch): FlowStruct = {
    flowStructLedger.getOrElseUpdate(
      (context, table.getName),
      new FlowStruct(context, table, switch)
    )
  }
}
