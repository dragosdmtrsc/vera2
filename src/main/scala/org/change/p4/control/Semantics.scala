...
abstract class Semantics[T](switch: Switch) {
  ...
  private val controlCFGs =
    mutable.Map.empty[String, LabeledGraph[ControlStatement, Option[P4BExpr]]]
  private val controlSCCs =
    mutable.Map.empty[String, List[List[ControlStatement]]]
  ...
  def translate(src: ControlStatement,
                rho: Option[P4BExpr],
  ...
  def deparse(startFrom: T): T = {
    ...
    runOnCFG("deparser", Map(first -> startFrom), lcfg)(
      new EndOfControl("deparser")
    )
  }

  def execute(
    control: String
  )(current: Map[ControlStatement, T]): Map[ControlStatement, T] = {
    val lcfg = getCFG(control)
