...
class SwitchTests extends FunSuite {
  ...
  test("SWITCH - code dump") {
    ...
    val switchInstance = SwitchInstance.fromP4AndDataplane(p4, dataplane, "switch", ifaces.foldLeft(new util.HashMap[Integer, String]())((acc, x) => {
      acc.put(x._1, x._2)
      acc
    }))

    //-i 0@veth0 -i 1@veth2 -i 2@veth4 -i 3@veth6 -i 4@veth8 -i 5@veth10 -i 6@veth12 -i 7@veth14 -i 8@veth16 -i 64@veth250
    val res = ControlFlowInterpreter(switchInstance,
      optAdditionalInitCode = None,
      optInitFactory = None,
      optTableFactory = None,
      optGenerator = Some(
        new SwitchBasedParserGenerator(switch = switchInstance.getSwitchSpec,
          switchInstance = switchInstance, codeFilter = Some((x : String) => {
            x.startsWith("parse_ethernet.parse_ipv4")
          }))
      )
    )
    val port = 0
